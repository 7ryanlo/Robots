...
bool Robot::getAttacked(int dir)  // return true if dies
{
    ...
    bool b = m_arena->determineNewPosition(m_row, m_col, dir);
...
void Player::moveOrAttack(int dir)
{
    ...
    {
        //cerr << "Playa has attacked/stood still" << endl;
        m_arena->attackRobotAt(r, c, dir);
    }
    else
    {
        //cerr << "Playa has moved position" << endl;
        m_arena->determineNewPosition(m_row, m_col, dir);
    }
}
...
void Arena::display() const
{
    ...
    for (int i = 0; i < m_nRobots; i ++)
    {
        ...
        {
            int nRobots = nRobotsAt(m_robots[i]->row(), m_robots[i]->col());
        
            if (nRobots == 1)
                grid[m_robots[i]->row() - 1][m_robots[i]->col() - 1] = 'R';
            else if (nRobots > 1 && nRobots < 9)
                grid[m_robots[i]->row() - 1][m_robots[i]->col() - 1] = nRobots + '0';
            else
                grid[m_robots[i]->row() - 1][m_robots[i]->col() - 1] = '9';
        }
    }
    ...
    for (int i = 0; i < robotCount(); i ++)
    {
        if (m_robots[i]->row() == r && m_robots[i]->col() == c)
        {
            if (m_robots[i]->getAttacked(dir))
            {
                ...
                for (int k = i; k < robotCount() - 1; k ++)
                {
                    m_robots[k] = m_robots[k + 1];
                }
                
                m_nRobots --;
                //cerr << "The robot " << i << " is dead" << endl;
                return true;
            }
            else
            {
                //cerr << "The robot " << i << " is still alive" << endl;
                return false;
            }
        }
    }
